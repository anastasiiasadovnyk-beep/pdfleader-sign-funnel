...
- `"nearestToken": true` on a region marks it as an approximation: the design value has **no exact ui-pes token**, so you asserted the nearest token's value instead of the design's. The gate still asserts and passes, but reports it separately (`⚠ N region(s) asserted against the nearest token`) so an approximation never hides inside the clean pass count. Use it whenever you round a design value to a token (e.g. design 39/800 → `text-desktop-title-2` 40/900) and record the real design value in `dsGap`/`note`. Don't assert the built value silently as if it matched.
