...
| No file-type glyph / thumbnail component, and no scalloped **seal** shape for a signed-document indicator | file glyph composed from a bordered box + folded-corner square + an extension chip; the digital seal is a round badge in the seal's green instead of the reference's scalloped mark | sign-funnel |
...
| `IconButton variant="outlined"` cannot draw a circle. `--radius-icon-btn-sm` is `1rem` and `--icon-btn-outlined-border-width` is `2px`, so the ring grows the box to 36px while the radius stays 16px — each side keeps a ~4px straight run (a squircle), and the outlined button ends up 4px larger than a `text`-variant sibling of the same `size`. **Upstream fix: make the icon-button radius `9999px`/half-size, or subtract the border from the box** | a ringed icon action cannot match a reference that draws a true circle, and mixing `text` and `outlined` icon buttons in one cluster gives two different box sizes | sign-funnel |
| No plain (weight-400) ~20px type token. The only `1.25rem` token is `--text-desktop-title-6` at weight 700; `--text-subtitle` is `1.125rem`/18px | a 20px regular nav/label from a reference has to settle for 18px `text-subtitle` or an arbitrary `text-[20px]` | sign-funnel |
...
| The `material-*` palette has no paired ink token, and the mid steps are too light for `text-common-white` | file-type chips (PDF/XLSX/DOCX/JPG/PNG) drawn white-on-colour measure ≈1.6:1 for amber-500, ≈3.7:1 for green-700 and ≈4.2:1 for red-600 — all under WCAG AA for that text size. The product reference draws them exactly this way, so sign-funnel keeps parity rather than deviating. **A `-contrast-text` companion per material step (or documented safe steps) would let concepts pick legible ink without guessing** | **needs DS decision** |
