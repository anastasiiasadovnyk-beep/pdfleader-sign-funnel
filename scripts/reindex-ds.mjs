...
import { parseCssVars, tailwindUtilFor, safelistClasses } from './lib/css-tokens.mjs';
...
const safelist = [...new Set(vars.flatMap((v) => safelistClasses(v.name)))].sort();
mkdirSync('src/styles', { recursive: true });
writeFileSync('src/styles/ds-safelist.css',
  `/* generated by reindex-ds from @universe-forma/ui-pes@${version}. DO NOT EDIT.\n` +
  `   Forces every ui-pes token utility to be emitted so token classes never silently\n` +
  `   no-op when a concept is the only place that uses them. */\n` +
  `@source inline("${safelist.join(' ')}");\n`);

console.log(`reindex-ds: wrote ds-catalog for ui-pes@${version} (${vars.length} tokens, ${new Set(exports).size} components, ${safelist.length} safelisted utilities)`);
