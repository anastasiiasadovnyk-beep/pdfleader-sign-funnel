...
## Purpose
The modal a pdfguru user sees after choosing **Compress** on a document: pick a compression preset (High / Medium / Low), or **Custom** with an adjustable slider, then confirm. Selecting a row switches the active mode; the Custom row reveals a slider; the CTA label updates live with the projected size. Built as an A/B-testable variant of the compress flow.
...
| `options[].recommended` | `boolean` | server-driven "recommended" preset flag from the estimate response |
...
| `orLabel` / `recommendedLabel` / `customSliderLabel` | `string` | `t('compress_modal.*')` |
...
| `onBack` | `() => void` | return to the previous tool step |
...
## States
- **Default** — a preset is selected (mock: Medium, `recommended`). Selected row: `border-primary` + `bg-primary-opacity-4`; savings label switches to `text-primary`.
- **Preset switch** — clicking High/Medium/Low moves selection and updates the CTA size.
- **Custom selected** — the Custom row reveals `QualitySlider`; dragging updates the Custom row's projected size and the CTA label in real time.
...
5. i18n: replace literals (`title`, `orLabel`, `recommendedLabel`, `customSliderLabel`, `ctaLabelTemplate`, `file.sizeLabel`) with `t('compress_modal.*')` keys in `src/locales/en/*`.
...
- **Icons** — ui-pes exports no icon set; chevrons/close/quality/sliders glyphs are inline SVGs (`stroke="currentColor"`, token `h-*/w-*` sizing) in `components/icons.tsx`.
