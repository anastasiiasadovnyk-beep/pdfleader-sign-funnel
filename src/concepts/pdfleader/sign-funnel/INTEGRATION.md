...
- **dashboard** — one page for both, built to the product's My Documents reference: six files, newest first, of which two are signed with different kinds so both indicators and both row menus are visible at once (W-9 digital on top, W-4 simple further down; the other four are ordinary uploads with no indicator). Rows carry the file glyph and name, the last-edit date stacked over its relative time, the size, then the actions — and no rule between rows, per the reference. Actions are edit, a Download pill and an overflow menu holding Duplicate and Delete plus "Download audit trail", which only a digital signature has. **The reference draws all three actions as bare icons; the Download pill is this prototype's own style, kept on request.** Its knock-on effect is the one place the layout departs from the reference: the Actions track has to be wide enough for the pill, so the Size column sits a little left of where the reference puts it, while Last Edit lands on the reference's position. Header and rows share one fixed-width column template (`ROW_GRID`), since separate grids with `auto` tracks would not line up.
...
- **`step` says which dialog is open, never whether anything is signed.** That is `signatures.length > 0`. Keeping the two apart is what makes dismissing a dialog structurally unable to discard placed work — the earlier single-signature model needed a "where does cancel return to" flag to avoid exactly that.
- **The placed signature is a selection.** A new one lands selected — blue frame, eight handles, red inner outline, plus the contextual toolbar. A pointer-down anywhere that is not a signature or its toolbar deselects: chrome and toolbar go, the signatures stay. Clicking any signature selects it (and can start a drag in the same gesture); the selected one renders above its siblings. Both pieces of signature UI carry `data-signature-ui`, which is how the document-level listener tells "inside" from "outside" — so toolbar controls don't deselect what they're editing. The toolbar is also gated on the editor step, so it never sits behind an open dialog and dialog clicks cannot reach the deselect listener.
...
- `signedUnverified` — one signature placed (`initialStep: 'signed'` seeds a single selected signature), contextual toolbar, Verified toggle **off** (Simple flow).
...
Interactions inside the page (view-model `hooks/useSignFunnelModel.ts`): Sign tool / purple `sign` field → select type (first signature only; later ones skip straight to the canvas) → Continue → create → tab switching, swatches, thickness, draw/type/upload fill, Clear, **Add signature** appends one — **Save** when the pencil opened it to edit. Each signature is selected, dragged between pages, recoloured and deleted on its own; toolbar toggles Verified; Done → `onNext`.
