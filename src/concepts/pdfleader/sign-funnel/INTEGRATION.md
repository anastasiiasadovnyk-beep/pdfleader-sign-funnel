...
- **dashboard** — one page for both, listing the two signed copies of the form so each indicator and each row menu is visible at once. The digital copy is the newer of the two, which keeps the grey (simple) row below the green (digital) one. Rows are laid out like the product's My Documents page: edit and a Download pill, with Duplicate and Delete behind the overflow menu — plus "Download audit trail", which only a digital signature has. Header and rows share one fixed-width column template (`ROW_GRID`), since separate grids with `auto` tracks would not line up.
