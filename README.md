...
Відкриється браузер із **галереєю** концептів (`http://localhost:5173`). Кожен концепт відкривається на весь екран за адресою `/c/<назва>`.
...
Структура кожного концепту (див. `src/concepts/_template/` як скелет, і `documents-empty/`, `document-detail/` як приклади):
